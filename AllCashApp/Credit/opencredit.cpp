...
OpenCredit::OpenCredit(QWidget *parent)
  ...
  connect(&manager, &FinanceManager::dataChanged, this,
          &OpenCredit::updateView);
  updateView();
}
...
void OpenCredit::updateView() {
  ui->creditAmountLabel->setText(
      QString::number(ui->creditAmountSlider->value()));
  ui->creditTermLabel->setText(QString::number(ui->creditTermSlider->value()));

  connect(ui->creditAmountSlider, &QSlider::valueChanged, this,
          &OpenCredit::on_creditAmountSlider_valueChanged);
  connect(ui->creditTermSlider, &QSlider::valueChanged, this,
          &OpenCredit::on_creditTermSlider_valueChanged);
}
...
void OpenCredit::on_openCreditButton_clicked() {
  ...
  if (сalculationStrategy) {
    double monthlyPayment =
        сalculationStrategy->calculate(creditAmount, 3.5, creditTerm * 12);
